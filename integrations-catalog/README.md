...
**540 integrations** across 9 categories. Auto-synced daily from production.
...
- **Vendor info:** name, slug, category, description, logo, docs URL, base URL
...
- **Capabilities:** whether employee sync is supported, multi-connection support
...
This catalog is auto-generated by `scripts/sync.mjs`, which runs on a daily GitHub Action. It pulls every integration from the CompAI internal API, strips implementation details, and writes the sanitized result here. Human edits to `integrations/*.json` will be overwritten on the next sync.

To request a new integration, open an issue with `[integration request]` in the title and the vendor name + docs URL.
