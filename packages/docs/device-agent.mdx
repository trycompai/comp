...
If you previously had the old Fleet-based agent (FleetDM) installed, you should uninstall it after setting up the new Comp AI Device Agent. The new agent fully replaces the old one.
  ...
  <Accordion title="macOS" icon="apple">
    ...
    **2. Remove the Fleet Desktop app**
    ...
    2. Open **Terminal** and run:
       ```bash
       sudo /opt/orbit/bin/orbit/orbit --remove
       ```
       If that doesn't work, manually remove the files:
       ```bash
       sudo rm -rf /opt/orbit
       sudo rm -rf /usr/local/bin/osqueryd
       sudo rm -f /Library/LaunchDaemons/com.fleetdm.orbit.plist
       sudo launchctl bootout system/com.fleetdm.orbit 2>/dev/null
       ```
    3. Restart your Mac to ensure all Fleet processes are stopped.
  </Accordion>

  <Accordion title="Windows" icon="windows">
    1. Open **Settings** → **Apps** → **Apps & features** (or **Installed apps** on Windows 11).
    2. Search for **Fleet osquery** or **Orbit**.
    3. Click **Uninstall** and follow the prompts.
    4. Restart your computer.

    If the app doesn't appear in the list, open **PowerShell as Administrator** and run:
    ```powershell
    msiexec /x "{fleet-product-code}" /quiet
    ```
    Or manually remove:
    ```powershell
    Stop-Service -Name "orbit" -Force -ErrorAction SilentlyContinue
    sc.exe delete "orbit"
    Remove-Item -Recurse -Force "C:\Program Files\Orbit" -ErrorAction SilentlyContinue
    ```
  </Accordion>

  <Accordion title="Linux" icon="linux">
    1. Stop the Fleet service:
       ```bash
    ...
    2. Remove the package:
       ```bash
       sudo apt remove fleet-osquery   # Debian/Ubuntu
       sudo rpm -e fleet-osquery       # RHEL/Fedora
       ```
