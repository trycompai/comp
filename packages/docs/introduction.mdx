---
title: "Welcome to Comp AI"
---

<img
  className="block dark:hidden"
  src="/packages/docs/CompAIbackground.jpeg"
  alt="Hero Light"
  title=""
/>

<img
  className="hidden dark:block"
  src="/packages/docs/CompAIbackground.jpeg"
  alt="Hero Dark"
  title=""
/>

## Quick Links

Resource links for Comp AI features

<CardGroup cols={2}>
  <Card title="Integrations" href="https://trycomp.ai/docs/cloud-tests">
    Connect to cloud providers such as AWS, GCP and Azure
  </Card>
  <Card
    title="Comp AI Device Agent"
    href="https://trycomp.ai/docs/device-agent"
  >
    Monitor your employee devices with ease with the Comp AI device agent
  </Card>
</CardGroup>

## Build with Comp AI

Comp AI API reference docs

<CardGroup cols={2}>
  <Card
    title="API Reference"
    icon="code"
    href="https://trycomp.ai/docs/api-reference/organization/get-organization-information"
  >
    Build internal tools to help manage evidence collection, policies, employees
    and more with the Comp AI API
  </Card>
</CardGroup>
