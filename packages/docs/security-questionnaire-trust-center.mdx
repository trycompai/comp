---
title: 'Security Questionnaire via Trust Access'
description: 'Use Trust Access tokens to let external reviewers upload questionnaires and receive AI-generated answers from approved Comp AI evidence.'
---

<Note>
  For internal organization users with full editing capabilities, see the main [Security
  Questionnaire](./security-questionnaire) documentation.
</Note>
...
<Note>
  Trust Access tokens are valid for the duration of your access grant (default 30 days, configurable
  7-365 days). If your token expires, you'll need to request new access through the Trust Access
  workflow.
</Note>
    ...
    <script>
      document.getElementById('questionnaireForm').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          alert(
            `Questionnaire processed! ${response.headers.get('X-Question-Count')} questions answered.`,
          );
        } catch (error) {
...
class SecurityQuestionnaireClient {
  ...
  async submitQuestionnaire(
    ...
    const blob = await response.blob();
    const questionCount = parseInt(response.headers.get('X-Question-Count') || '0', 10);
...
2. **API Reference:** Check the [questionnaire API reference](/api-reference/questionnaire/upload-and-auto-answer-a-questionnaire-via-trust-portal-token) for detailed endpoint specifications
