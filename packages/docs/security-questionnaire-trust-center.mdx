---
title: 'Security Questionnaire Trust Center'
description: 'A comprehensive guide to using the Security Questionnaire API for external users via Trust Access tokens.'
---

<Note>
  For internal organization users with full editing capabilities, see [Security Questionnaire](./security-questionnaire).
</Note>
