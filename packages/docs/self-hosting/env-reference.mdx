...
| `RESEND_API_KEY` | app, portal | runtime | [Resend](https://resend.com) API key for email (or define a custom SMTP relay; see below) |
...
<Note>
  Email configuration is mandatory for the application to work.
  You can either use Resend by providing an API key or use a custom SMTP-relay.
  If both are configured, Resend will be used.
</Note>
...
| Name | Service(s) | Phase | Required | Notes                     |
|------|-----------|-------|----------|---------------------------|
| `RELAY_SMTP_HOST` | app, portal, api | runtime | required-minimal | SMTP host name            |
| `RELAY_SMTP_PORT` | app, portal, api | runtime | required-minimal | SMTP host port            |
| `RELAY_SMTP_USER` | app, portal, api | runtime | required-minimal | SMTP username             |
| `RELAY_SMTP_PASS` | app, portal, api | runtime | required-minimal | password of the SMTP user |
...
<Note>
  AWS S3 variables are required for file upload features (attachments, logos, questionnaires). Without them, these
  features will fail.
</Note>
