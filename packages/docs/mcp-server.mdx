...
<Note>
The `npx -y @gideon-defender/mcp-server` command you'll see throughout is what handles the download. Your AI client runs it in the background when you start a chat; npm fetches the latest published version from [npmjs.com](https://www.npmjs.com/package/@gideon-defender/mcp-server) and caches it locally. Nothing for you to install manually.
</Note>
...
```bash
claude mcp add CompAI -- npx -y @gideon-defender/mcp-server start --apikey comp_your_api_key_here
```
...
```bash
codex mcp add comp-ai -- npx -y @gideon-defender/mcp-server start --apikey comp_your_api_key_here
```
...
```bash
gemini mcp add CompAI -- npx -y @gideon-defender/mcp-server start --apikey comp_your_api_key_here
```
...
- **npm package:** [`@gideon-defender/mcp-server`](https://www.npmjs.com/package/@gideon-defender/mcp-server)
