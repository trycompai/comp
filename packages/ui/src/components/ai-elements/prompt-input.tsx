  ...
  const inner = (
    <>
      ...
      <form className={cn('w-full', className)} onSubmit={handleSubmit} {...props}>
        <InputGroup
          className={cn(
            'overflow-hidden rounded-none border-x-0 border-b-0 border-t border-border/70 bg-background/95 shadow-none',
          )}
        >
          {children}
        </InputGroup>
...
export const PromptInputTextarea = ({
  ...
  const handleKeyDown: KeyboardEventHandler<HTMLTextAreaElement> = (e) => {
    if (e.key === 'Enter') {
      ...
      // Require Cmd+Enter (Mac) or Ctrl+Enter (Windows/Linux) to submit
...
export const PromptInputSubmit = ({
  ...
  if (status === 'error') {
    ...
    // Show keyboard shortcut hint: modifier + arrow
    const isMac =
      typeof window !== 'undefined' && navigator.platform.toUpperCase().indexOf('MAC') >= 0;
    content = (
      <span className="text-xs leading-none text-primary-foreground/90 font-medium flex items-center gap-1">
        {isMac ? '⌘' : 'Ctrl'}
  ...
  return (
    <InputGroupButton
      aria-label="Submit"
      className={cn('!h-8 !px-2 flex items-center justify-center', className)}
      size="xs"
      type="submit"
      variant={variant}
      {...props}
