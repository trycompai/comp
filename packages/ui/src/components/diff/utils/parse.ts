...
const buildInlineDiffSegments = (
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (current && current.type === 'delete' && next?.type === 'insert') {
      ...
      mergeIntoResult(current as LineSegment);
    }
  }
...
const mergeAdjacentLines = (changes: _Change[], options: ParseOptions): Line[] => {
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (
      ...
      out.push(changeToLine(current as _Change));
    }
  }
...
function buildChangeIndices(changes: _Change[]) {
  ...
  for (let i = 0; i < changes.length; i++) {
    const c = changes[i];
    if (c && c.type === 'insert') insertIdxs.push(i);
    else if (c && c.type === 'delete') deleteIdxs.push(i);
  }
...
function buildUnpairedDeletePrefix(changes: _Change[], pairOfDel: Int32Array) {
  ...
  for (let i = 0; i < n; i++) {
    const c = changes[i];
    const isInitiallyUnpairedDelete = c && c.type === 'delete' && pairOfDel[i] === UNPAIRED;
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (c && c.type === 'normal') {
      ...
      if (pairedDelIdx === UNPAIRED) {
        ...
        emitModified(out, del, c! as InsertChange, options);
        processed[i] = 1;
