      ...
      if (!hasEncryption) {
        findings.push(
          this.finding(vm, {
            ...
            severity: 'low',
            remediation:
              'Enable disk encryption with a customer-managed key via Azure Disk Encryption or Disk Encryption Sets.',
          }),
        );
      }
      ...
      if (linuxConfig && linuxConfig.disablePasswordAuthentication === false) {
        findings.push(
          this.finding(vm, {
            ...
            severity: 'medium',
            remediation:
              'Disable password authentication and use SSH key-based authentication only.',
          }),
        );
      }
    ...
    if (findings.length === 0) {
      findings.push({
        ...
        evidence: {
          ...
          findingKey: 'azure-virtual-machine-all-ok',
        },
