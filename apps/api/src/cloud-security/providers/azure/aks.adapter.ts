...
export class AksAdapter implements AzureServiceAdapter {
  ...
  async scan({
    ...
    for (const cluster of clusters) {
      ...
      if (props.enableRBAC !== true) {
        findings.push(
          this.finding(cluster, {
            ...
            severity: 'critical',
            remediation:
              'Enable Kubernetes RBAC. Note: this requires cluster recreation for existing clusters.',
          }),
        );
      }
      ...
      if (!props.networkProfile?.networkPolicy) {
        findings.push(
          this.finding(cluster, {
            ...
            severity: 'high',
            remediation:
              'Enable Azure or Calico network policy plugin. Note: requires cluster recreation.',
          }),
        );
      }
      ...
      if (
        !apiAccess?.enablePrivateCluster &&
        (!apiAccess?.authorizedIPRanges ||
          apiAccess.authorizedIPRanges.length === 0)
      ) {
        findings.push(
          this.finding(cluster, {
            ...
            severity: 'high',
            remediation:
              'Enable private cluster or configure authorized IP ranges for the API server.',
          }),
        );
      }
      ...
      if (!upgradeChannel || upgradeChannel === 'none') {
        findings.push(
          this.finding(cluster, {
            ...
            severity: 'medium',
            remediation:
              'Set auto-upgrade channel to "patch" or "stable" for automatic security updates.',
          }),
        );
      }
      ...
      if (!props.addonProfiles?.omsagent?.enabled) {
        findings.push(
          this.finding(cluster, {
            ...
            severity: 'medium',
            remediation:
              'Enable the monitoring addon to collect container logs and metrics.',
          }),
        );
      }
    }
