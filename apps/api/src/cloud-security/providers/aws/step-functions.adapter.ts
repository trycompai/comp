      ...
      do {
        ...
        for (const sm of listRes.stateMachines ?? []) {
          ...
          if (descRes.tracingConfiguration?.enabled !== true) {
            findings.push(
              this.makeFinding(smArn, 'X-Ray tracing not enabled', `Step Functions state machine "${smName}" does not have X-Ray tracing enabled`, 'low', { stateMachineName: smName, tracingEnabled: descRes.tracingConfiguration?.enabled }, false, `Use sfn:UpdateStateMachineCommand with stateMachineArn set to '${smArn}' and tracingConfiguration set to { enabled: true }. Ensure the state machine's IAM role has xray:PutTraceSegments and xray:PutTelemetryRecords permissions. Rollback: use sfn:UpdateStateMachineCommand with tracingConfiguration.enabled set to false.`),
            );
          } else {
            findings.push(
              this.makeFinding(smArn, 'X-Ray tracing enabled', `Step Functions state machine "${smName}" has X-Ray tracing enabled`, 'info', { stateMachineName: smName, tracingEnabled: true }, true),
            );
          }
          ...
          if (encType && encType !== 'CUSTOMER_MANAGED_KMS_KEY') {
            findings.push(
              this.makeFinding(smArn, 'State machine using AWS-managed encryption key', `Step Functions state machine "${smName}" uses an AWS-managed key instead of a customer-managed KMS key`, 'low', { stateMachineName: smName, encryptionType: encType }, false, `Use sfn:UpdateStateMachineCommand with stateMachineArn set to '${smArn}' and encryptionConfiguration set to { type: 'CUSTOMER_MANAGED_KMS_KEY', kmsKeyId: '<kms-key-arn>', kmsDataKeyReusePeriodSeconds: 300 }. Rollback: use sfn:UpdateStateMachineCommand with encryptionConfiguration.type set to 'AWS_OWNED_KEY'.`),
            );
          } else if (encType === 'CUSTOMER_MANAGED_KMS_KEY') {
            findings.push(
              this.makeFinding(smArn, 'State machine using customer-managed KMS key', `Step Functions state machine "${smName}" uses a customer-managed KMS key for encryption`, 'info', { stateMachineName: smName, encryptionType: encType }, true),
            );
          } else {
            findings.push(
              this.makeFinding(smArn, 'State machine using AWS-managed encryption key', `Step Functions state machine "${smName}" uses default AWS-managed encryption`, 'low', { stateMachineName: smName, encryptionType: 'AWS_OWNED_KEY' }, false, `Use sfn:UpdateStateMachineCommand with stateMachineArn set to '${smArn}' and encryptionConfiguration set to { type: 'CUSTOMER_MANAGED_KMS_KEY', kmsKeyId: '<kms-key-arn>', kmsDataKeyReusePeriodSeconds: 300 }. Rollback: use sfn:UpdateStateMachineCommand with encryptionConfiguration.type set to 'AWS_OWNED_KEY'.`),
            );
          }
        }
