...
export class DevicesService {
  ...
  async findAllByMember(organizationId: string, memberId: string): Promise<DeviceResponseDto[]> {
    try {
  ...
  async getMemberById(organizationId: string, memberId: string): Promise<MemberResponseDto> {
    try {
      const member = await db.member.findFirst({
        where: { 
          id: memberId,
