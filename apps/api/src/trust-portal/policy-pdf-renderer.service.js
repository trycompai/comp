...
exports.PolicyPdfRendererService = void 0;
const common_1 = require("@nestjs/common");
const jspdf_1 = require("jspdf");
let PolicyPdfRendererService = class PolicyPdfRendererService {
            ...
            switch (node.type) {
                case 'heading':
                    ...
                    if (node.content) {
                        ...
                        const lines = config.doc.splitTextToSize(headingText, config.contentWidth);
                        for (const line of lines) {
                            this.checkPageBreak(config);
                            config.doc.text(line, config.margin, config.yPosition);
                    ...
                    if (node.content) {
                        node.content.forEach((item, index) => {
                            if (item.type === 'listItem' && item.content) {
                                ...
                                const itemText = this.cleanTextForPDF(this.extractTextFromContent(item.content));
                                const lines = config.doc.splitTextToSize(itemText, config.contentWidth - 10);
                                config.doc.text(bullet, config.margin, config.yPosition);
                                for (let i = 0; i < lines.length; i++) {
                                    this.checkPageBreak(config);
                                    config.doc.text(lines[i], config.margin + 10, config.yPosition);
                                    config.yPosition += config.lineHeight;
                                }
                                if (item.content.some((n) => n.type === 'bulletList' || n.type === 'orderedList')) {
                                    const nestedLists = item.content.filter((n) => n.type === 'bulletList' || n.type === 'orderedList');
                ...
                case 'codeBlock':
                    ...
                    if (node.content) {
                        ...
                        const lines = config.doc.splitTextToSize(codeText, config.contentWidth - 10);
                        for (const line of lines) {
                            this.checkPageBreak(config);
                    ...
                    if (node.content) {
                        for (const quoteNode of node.content) {
                            if (quoteNode.content) {
                                const quoteText = this.cleanTextForPDF(this.extractTextFromContent(quoteNode.content));
                                const lines = config.doc.splitTextToSize(quoteText, config.contentWidth - 15);
                                for (const line of lines) {
                                    this.checkPageBreak(config);
    ...
    renderPoliciesPdfBuffer(policies, organizationName) {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            doc.text(`Page ${i} of ${totalPages}`, config.pageWidth / 2, config.pageHeight - 10, { align: 'center' });
        }
        const arrayBuffer = doc.output('arraybuffer');
        return Buffer.from(arrayBuffer);
    }
};
exports.PolicyPdfRendererService = PolicyPdfRendererService;
exports.PolicyPdfRendererService = PolicyPdfRendererService = __decorate([
    (0, common_1.Injectable)()
], PolicyPdfRendererService);
//# sourceMappingURL=policy-pdf-renderer.service.js.map