...
function validateSecurityConfig(): void {
  if (!process.env.AUTH_SECRET) {
    throw new Error(
      'SECURITY ERROR: AUTH_SECRET environment variable is required. ' +
  ...
  if (process.env.AUTH_SECRET.length < 32) {
    throw new Error(
      'SECURITY ERROR: AUTH_SECRET must be at least 32 characters long for security.'
    );
  }
...
export const auth = betterAuth({
  ...
  baseURL: process.env.AUTH_BASE_URL || process.env.BETTER_AUTH_URL || 'http://localhost:3000',
  ...
  // AUTH_SECRET is validated at startup via validateSecurityConfig()
