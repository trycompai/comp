...
export class PeopleService {
  ...
  async findById(memberId: string, organizationId: string): Promise<PeopleResponseDto> {
    try {
      await MemberValidator.validateOrganization(organizationId);
      const member = await MemberQueries.findByIdInOrganization(memberId, organizationId);
  ...
  async create(organizationId: string, createData: CreatePeopleDto): Promise<PeopleResponseDto> {
    try {
      ...
      await MemberValidator.validateUserNotMember(createData.userId, organizationId);

      const member = await MemberQueries.createMember(organizationId, createData);
  ...
  async updateById(memberId: string, organizationId: string, updateData: UpdatePeopleDto): Promise<PeopleResponseDto> {
    try {
      await MemberValidator.validateOrganization(organizationId);
      const existingMember = await MemberValidator.validateMemberExists(memberId, organizationId);
      ...
      if (updateData.userId && updateData.userId !== existingMember.userId) {
        ...
        await MemberValidator.validateUserNotMember(updateData.userId, organizationId, memberId);
      }

      const updatedMember = await MemberQueries.updateMember(memberId, updateData);
  ...
  async deleteById(memberId: string, organizationId: string): Promise<{ success: boolean; deletedMember: { id: string; name: string; email: string } }> {
    try {
      await MemberValidator.validateOrganization(organizationId);
      const member = await MemberQueries.findMemberForDeletion(memberId, organizationId);
