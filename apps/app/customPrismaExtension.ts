...
export class PrismaExtension implements BuildExtension {
  ...
  async onBuildStart(context: BuildContext) {
    ...
    while (
      !existsSync(dbPackagePath) &&
      workspaceRoot !== dirname(workspaceRoot)
    ) {
      workspaceRoot = dirname(workspaceRoot);
      dbPackagePath = resolve(
        workspaceRoot,
        "node_modules/@trycompai/db/dist/schema.prisma",
      );
    }
    ...
    try {
      ...
      context.logger.debug(
        `@trycompai/db package directory contents:`,
        readdirSync(dbPackageDir),
      );
      context.logger.debug(
        `@trycompai/db/dist directory contents:`,
        readdirSync(dbDistDir),
      );
    } catch (err) {
