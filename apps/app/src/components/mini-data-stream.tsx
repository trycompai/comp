...
export function MiniDataStream({ taskType, itemTitle }: MiniDataStreamProps) {
  const [lines, setLines] = useState(() =>
    Array.from({ length: NUM_LINES }, () =>
      generateRelevantLine(taskType, itemTitle)
    )
  );
  ...
  return (
    <div
      className="bg-muted/30 relative h-14 w-full overflow-hidden rounded-md font-mono text-xs leading-relaxed"
      aria-hidden="true"
    >
      <motion.div
        animate={{
