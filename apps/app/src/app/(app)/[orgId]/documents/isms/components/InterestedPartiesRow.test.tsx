...
describe('InterestedPartiesRow', () => {
  ...
  it('re-syncs the form from a changed record while not editing', async () => {
    ...
    // Updated record arrives (e.g. after a revalidation) while in read mode. The
    // form re-seeds via reset() inside an effect, so settle that under act.
    await act(async () => {
      rerender(
        <InterestedPartiesRow
  ...
  it('gates Save until the form is dirty and valid', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(
      <InterestedPartiesRow
        party={makeParty()}
    ...
    fireEvent.click(screen.getByLabelText('Edit interested party'));
    // Clearing a required field makes the form invalid; saving is a no-op.
    fireEvent.change(screen.getByLabelText('Interested party name'), {
      target: { value: '' },
    });
    fireEvent.click(screen.getByText('Save'));
