...
async function* getCommandLogs(sandboxId: string, cmdId: string) {
  const response = await fetch(`/api/tasks-automations/sandboxes/${sandboxId}/cmds/${cmdId}/logs`, {
...
async function getCommand(sandboxId: string, cmdId: string) {
  const response = await fetch(`/api/tasks-automations/sandboxes/${sandboxId}/cmds/${cmdId}`);
