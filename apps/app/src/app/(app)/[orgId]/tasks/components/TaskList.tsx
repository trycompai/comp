...
export function TaskList({
  ...
  const overallStats = useMemo(() => {
    ...
    const inProgress = initialTasks.filter(
    ...
    const enabledAutomations = initialTasks.reduce((acc, t) => {
      return (
        acc + (t.evidenceAutomations?.filter((a) => a.isEnabled).length ?? 0)
      );
    }, 0);
    ...
    const automationRate =
      total > 0 ? Math.round((tasksWithAutomation / total) * 100) : 0;
    const successRate =
      totalRuns > 0 ? Math.round((successfulRuns / totalRuns) * 100) : 0;
    const avgRunDuration =
      runsWithDuration > 0
        ? Math.round(totalRunDuration / runsWithDuration)
        : 0;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        {initialTasks.length > 0 && (
          <>
            <div className="space-y-2 lg:col-span-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="text-muted-foreground flex items-center justify-between text-[10px] tabular-nums">
                  ...
                  <span>
                    {overallStats.total - overallStats.done} remaining
                  </span>
                </div>
              </div>
            </div>
            ...
            {/* Status Breakdown */}
            <div className="grid grid-cols-3 gap-3 lg:col-span-6 lg:pl-3 xl:pl-4">
              {/* Completed */}
              <div className="border-l-primary bg-card/50 border-l-2 px-3 py-2">
                <div className="text-muted-foreground mb-1 text-[10px] font-medium tracking-wider uppercase">
                  Done
                </div>
                <div className="text-foreground mb-0.5 text-xl font-semibold tabular-nums">
                  {overallStats.done}
                </div>
              ...
              {/* In Progress */}
              <div className="bg-card/50 border-l-2 border-l-blue-500 px-3 py-2">
                <div className="text-muted-foreground mb-1 text-[10px] font-medium tracking-wider uppercase">
                  Active
                </div>
                <div className="text-foreground mb-0.5 text-xl font-semibold tabular-nums">
                ...
                <div className="text-muted-foreground text-[10px] tabular-nums">
                  {overallStats.total > 0
                    ? Math.round(
                        (overallStats.inProgress / overallStats.total) * 100,
                      )
                    : 0}
                  %
                </div>
              ...
              {/* To Do */}
              <div className="border-l-muted-foreground/30 bg-card/50 border-l-2 px-3 py-2">
                <div className="text-muted-foreground mb-1 text-[10px] font-medium tracking-wider uppercase">
                  Pending
                </div>
                <div className="text-foreground mb-0.5 text-xl font-semibold tabular-nums">
        ...
        <div
          className={`border-l-primary/30 bg-card/50 border-l-2 px-3 py-2.5 lg:col-span-3 ${initialTasks.length === 0 ? "lg:col-span-12" : ""}`}
        >
          <div className="space-y-2.5">
            ...
            <div className="space-y-2">
              ...
              {overallStats.recentRuns24h > 0 && (
                <div className="border-border/50 bg-muted/10 text-muted-foreground flex items-center gap-2 rounded-sm border px-2 py-1.5 text-[10px]">
                  <div className="flex h-2 w-2 items-center justify-center">
                    <div className="bg-primary h-1.5 w-1.5 rounded-full" />
                  </div>
                  <span className="text-foreground tabular-nums">
                    {overallStats.recentRuns24h}
                  </span>
                  <span className="text-muted-foreground/60 tracking-[0.2em] uppercase">
                    runs in last 24h
                  </span>
                </div>
              )}
      ...
      <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {selectedMember.user.image ? (
                      <Image
                        ...
                        width={20}
                        height={20}
                        className="border-border/60 h-5 w-5 rounded-full border object-cover"
                      />
                    ) : (
                      <span className="border-border/60 bg-muted/40 text-muted-foreground flex h-5 w-5 items-center justify-center rounded-full border text-[10px] font-medium uppercase">
          ...
          <SelectContent>
            ...
            {eligibleAssignees.map((member) => (
              <SelectItem key={member.id} value={member.id}>
                <div className="flex items-center gap-2 text-sm">
                  {member.user.image ? (
                    <div className="border-border/60 bg-muted/40 flex h-6 w-6 items-center justify-center overflow-hidden rounded-full border">
                      <Image
                    ...
                    <span className="border-border/60 bg-muted/40 text-muted-foreground flex h-6 w-6 items-center justify-center rounded-full border text-[10px] font-medium uppercase">
        ...
        {/* View Toggle */}
        <div className="border-border bg-card flex items-center gap-0 rounded-md border p-0.5">
          <button
        ...
        {(searchQuery || statusFilter || assigneeFilter) && (
          <div className="flex items-center gap-2 px-3">
            ...
            <div className="text-muted-foreground text-xs tabular-nums">
              ...
              {filteredTasks.length === 1 ? "result" : "results"}
            </div>
          </div>
        )}
      </div>
