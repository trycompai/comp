...
export async function getRelevantTasksForIntegration(
  ...
  { taskTemplateId: string; taskName: string; reason: string; prompt: string }[]
> {
...
Return ONLY clearly relevant tasks as an ARRAY. Format: {"relevantTasks": [{"taskTemplateId": "...", "taskName": "...", "reason": "...", "prompt": "..."}, ...]}`;
  ...
  console.log(
    `[getRelevantTasks] Prompt size: ${promptSize} chars, ${taskTemplates.length} tasks`,
  );
        ...
        if (errorText) {
          ...
          if (parsed.relevantTasks) {
            ...
            if (tasks.length > 0 && tasks[0].taskTemplateId) {
              console.log(
                `[getRelevantTasks] Recovered ${tasks.length} tasks from error response`,
              );
              return tasks;
            }
          }
