...
export function EmployeeBackgroundCheck({
	...
	const [wizardStep, setWizardStep] = useState<"overview" | "details">(() => {
		if (searchParams.get("background_check_step") === "details")
			return "details";
	...
	const [requestConfirmation, setRequestConfirmation] = useState<string | null>(
		null,
	);
		...
		async (values: BackgroundCheckFormValues): Promise<boolean> => {
			...
			if (response.error || !response.data) {
				if (response.status === 402) {
					setPaymentIssue(
						"Payment failed. Update payment method and try again.",
					);
					return false;
				}
	...
	useEffect(() => {
		...
		const setupSucceeded =
			searchParams.get("background_check_billing") === "success";
		if (!sessionId || !setupSucceeded || handledSessionId.current === sessionId)
			return;
