...
/**
 * Generates a training completion certificate PDF with Comp AI branding and returns it as base64
 */
async function generateTrainingCertificatePdf(params: {
  userName: string;
  ...
  // Employee name
  const cleanUserName = cleanTextForPDF(userName);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(22);
  doc.setTextColor(18, 18, 18);
  ...
  doc.text('has successfully completed all modules of the', pageWidth / 2, 110, {
    align: 'center',
  });
