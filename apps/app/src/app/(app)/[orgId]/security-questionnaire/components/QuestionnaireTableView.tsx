...
export interface QuestionnaireResultsTableProps {
  orgId: string;
  ...
  recentlyUpdatedQuestions: Set<number>;
  savingState?: {
    isSaving: boolean;
    index: number | null;
  };
...
export const QuestionnaireResultsTable: FC<QuestionnaireResultsTableProps> = ({
  orgId,
  results,
  filteredResults,
  editingIndex,
      ...
      <Table>
        ...
        <TableBody>
          {filteredResults.map((qa, index) => {
            ...
            const isRecentlyUpdated = recentlyUpdatedQuestions.has(safeIndex);
            const isSavingActive =
              savingState?.isSaving && savingState.index !== null && savingState.index === safeIndex;
                  ...
                  {isEditing ? (
                    ...
                    <div className="space-y-3">
                      {qa.answer && qa.answer.trim().length > 0 ? (
                        <div 
                          className={cn(
                            'group relative cursor-pointer rounded-md p-2 -m-2 border border-transparent transition-colors duration-150 ease-in-out hover:bg-muted/30 hover:border-primary/30',
                            isRecentlyUpdated &&
                              'bg-emerald-500/5 dark:bg-emerald-400/5 border-emerald-400/40 hover:border-emerald-400/60 ring-1 ring-emerald-400/40 animate-[pulse_1.5s_ease-out_1]',
                          )}
