...
import { db, Departments } from "@trycompai/db";
...
export async function POST(request: NextRequest) {
  ...
  if (process.env.E2E_TEST_MODE !== "true") {
    console.log(
      "[TEST-LOGIN] E2E_TEST_MODE is not true:",
...
async function handleLogin(request: NextRequest) {
  ...
  console.log(
    "[TEST-LOGIN] Attempting sign in for user:",
    email,
    "with password:",
    testPassword,
  );
  ...
  if (!signInResponse.ok) {
    ...
    try {
      ...
      console.log(
        "[TEST-LOGIN] Sign in successful, user:",
  ...
  if (!body.skipOrg) {
    ...
    try {
      ...
      if (!setActiveOrgResponse.ok) {
        console.log(
          "[TEST-LOGIN] Warning: setActiveOrganization returned non-ok status",
        );
