...
export async function extractVendorsFromContext(
  ...
  const { object } = await generateObject({
    ...
    system: [
      ...
      '',
      'INHERENT RISK CALIBRATION — apply consistently. Inherent = risk before the customer applies any compensating controls; it reflects the vendor\'s own posture + the typical data they handle for this kind of customer. Without explicit calibration the model has been defaulting every vendor to (very_likely, severe) → 10/10, which is wrong.',
      '',
      ...
      '- very_unlikely: hyperscaler-tier vendor with multiple independent attestations (SOC 2 Type II + ISO 27001 + multi-region availability) and no major incidents in recent history (e.g. AWS, Google Cloud, Azure).',
      '- unlikely: well-established SaaS vendor with at least one strong attestation (SOC 2 Type II OR ISO 27001) and a clean public security history (e.g. GitHub, Stripe, Linear, Slack, Notion, Figma, HubSpot).',
      '- possible: established vendor without independent attestation, or with minor incidents in the last few years. This is the DEFAULT when you have no specific reason to deviate.',
      '- likely: vendor with public knowledge of significant security incidents within the last 24 months OR no attestations and limited transparency.',
      '- very_likely: vendor with chronic / systemic security issues, repeated breaches, or essentially unknown posture with sensitive data exposure.',
      '',
      'inherent_impact — business impact if the vendor is compromised, assuming average customer usage:',
      '- insignificant: no PII, no business data, purely cosmetic / public utility.',
      '- minor: anonymous usage metadata or non-business utilities.',
      '- moderate: vendor handles internal business data or basic PII (names, emails) but NOT payment / health / source / auth data. This is the DEFAULT for typical SaaS.',
      ...
      '',
      'residual_probability / residual_impact: leave at the same level as inherent unless the user\'s answers describe specific compensating controls already in place. Do NOT discount for the vendor\'s OWN controls (those are already factored into inherent above).',
      '',
      'Sanity check: most well-known SaaS vendors (GitHub, Stripe, Linear, Notion, Slack, Figma, etc.) should land at (unlikely, moderate) → ~3/10, NOT (very_likely, severe) → 10/10.',
    ].join('\n'),
