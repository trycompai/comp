...
export const generateFullPolicies = task({
  ...
  run: async (payload: { organizationId: string }) => {
    logger.info(
      `Starting full policy generation for organization ${payload.organizationId}`,
    );

    try {
      // Get organization context
      const { questionsAndAnswers } = await getOrganizationContext(
      ...
      logger.error(
        `Error during policy generation for organization ${payload.organizationId}:`,
        {
