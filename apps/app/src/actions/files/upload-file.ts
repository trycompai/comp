...
export const uploadFile = async (
  ...
  try {
    ...
    if (!s3Client) {
      logger.error(
        "[uploadFile] S3 client not initialized - check environment variables",
      );
      return {
        success: false,
        error:
    ...
    logger.info(
      `[uploadFile] Starting upload for ${fileName} in org ${organizationId}`,
    );
    ...
    logger.info(
      `[uploadFile] Creating attachment record in DB for key: ${key}`,
    );
