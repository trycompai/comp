...
async function main() {
  ...
  try {
    if (organizationId) {
      console.log(
        `🚀 Triggering training video backfill for organization: ${organizationId}`,
      );
      ...
      console.log(
        `📊 You can monitor the progress in the Trigger.dev dashboard`,
      );
    } else {
      console.log(
        "🚀 Triggering training video backfill for ALL organizations",
      );
