...
test.describe("Split Onboarding Flow", () => {
  ...
  test("new user without access: 3 steps → book call", async ({ page }) => {
    ...
    await expect(
      page.getByText("Which compliance frameworks do you need?"),
    ).toBeVisible();
    ...
    await page
      .getByPlaceholder("e.g., Acme Inc.")
      .fill(testData.organizationName);
    await page.getByTestId("setup-next-button").click();
    ...
    await expect(
      page.getByText(`Let's get ${testData.organizationName} approved`),
    ).toBeVisible();
    await expect(
      page.getByText("A quick 20-minute call with our team"),
    ).toBeVisible();
  });

  test("user creates org → book call → gets access → onboarding", async ({
    page,
  }) => {
    const testData = generateTestData();
    const website = `example${Date.now()}.com`;
    ...
    await expect(
      page.getByText("Describe your company in a few sentences"),
    ).toBeVisible();
  });

  test("user with approved org: redirected from product to onboarding", async ({
    page,
  }) => {
    const testData = generateTestData();
    const website = `example${Date.now()}.com`;
  ...
  test("completed user redirected from setup to app", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="onboarding-option-saas"]', {
      state: "visible",
    });
    ...
    await page.waitForSelector('[data-testid="onboarding-option-11-50"]', {
      state: "visible",
    });
    ...
    await page
      .getByTestId(
        "onboarding-input-devices-search-option-company-provided-laptops",
      )
      .click();
    await page.getByTestId("onboarding-next-button").click();
    ...
    await page
      .getByTestId(
        "onboarding-input-authentication-search-option-google-workspace",
      )
      .click();
    await page.getByTestId("onboarding-next-button").click();
    ...
    await page.waitForSelector(
      '[data-testid="onboarding-input-software-search-option-github"]',
      {
        state: "visible",
      },
    );
    await page
      .getByTestId("onboarding-input-software-search-option-github")
      .click();
    await page.getByTestId("onboarding-next-button").click();
    ...
    await page.waitForSelector(
      '[data-testid="onboarding-option-fully-remote"]',
      {
        state: "visible",
      },
    );
    ...
    await page
      .getByTestId("onboarding-input-infrastructure-search-option-aws")
      .click();
    await page.getByTestId("onboarding-next-button").click();
    ...
    await page
      .getByTestId("onboarding-input-dataTypes-search-option-customer-pii")
      .click();
  ...
  test("user without access gets blocked then granted access", async ({
    page,
  }) => {
    const testData = generateTestData();
    const website = `example${Date.now()}.com`;
