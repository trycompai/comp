...
test.describe.skip("Onboarding Flow (DEPRECATED)", () => {
  ...
  test("new user can complete full onboarding flow", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/compliance frameworks/i").first(),
    ).toBeVisible({
      timeout: 10000,
    });
  ...
  test("existing user can create additional organization", async ({ page }) => {
    ...
    while (attempts < maxAttempts) {
      ...
      const pills = await page
        .locator('[role="button"][data-value]:visible')
        .count();
      if (pills > 0 && pills < 20) {
        const pillButtons = await page
          .locator('[role="button"][data-value]:visible')
          .all();
  ...
  test("user without org is redirected to setup from dashboard", async ({
    page,
  }) => {
    const testData = generateTestData();
  ...
  test("handles organization creation errors gracefully", async ({ page }) => {
    ...
    await fillFormField(
      page,
      'textarea[name="describe"]',
      "Existing Org is a test company.",
    );
  ...
  test("framework selection enables/disables Next button", async ({ page }) => {
    ...
    if (allFrameworkLabels.length > 1) {
      ...
      if (hasUnchecked) {
        ...
        const newCheckedCount = await page
          .locator('input[type="checkbox"]:checked')
          .count();
