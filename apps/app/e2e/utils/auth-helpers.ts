...
export async function grantAccess(
  ...
  console.log(
    `Granting access to organization: ${orgId}, hasAccess: ${hasAccess}`,
  );

  const response = await page
    .context()
    .request.post("/api/auth/test-grant-access", {
  ...
  if (!response.ok()) {
    const errorBody = await response.text();
    throw new Error(
      `Failed to grant access: ${response.status()} - ${errorBody}`,
    );
  }
