...
> {
  const parsed$ = safeParse(
    request,
    (value$) =>
      EvidenceExportControllerExportTaskEvidenceZipV1Request$zodSchema.parse(
        value$,
      ),
  ...
  const path$ = pathToFunc("/v1/tasks/{taskId}/evidence/export")(
    pathParams$,
  );
  const query$ = encodeFormQuery({
    "includeJson": payload$.includeJson,
  });
